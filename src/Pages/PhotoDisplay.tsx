...
function PhotoDisplay({ photodata }: { photodata: { title: string; thumbnail: string; src: Array<string>; categories: Array<string> } }) {
  ...
  const newIndex = direction === "right"
    ? Math.min(currentIndex + 1, photodata.src.length - 1)
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const childWidth = scrollContainer.scrollWidth / photodata.src.length;
